...
# step 1: no mask
	...
	bestlinreg_s -lsq12 -nmi $tmp/normal1.mnc $template $tmp/lin1.xfm
...
if [ $reg -eq 1 ]; then
    bestlinreg_s -nmi -lsq12 $tmp/normal2.mnc $template $tmp/lin2.xfm -source_mask $tmp/mask.mnc -target_mask $mask -clob
